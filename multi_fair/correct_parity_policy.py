...
def correct_parity_policy(ranking, group_info, pthres, ithres, atr_correct, inter_given):
    """   A function for correcting the parity of a ranking
                ...
                :param atr_correct: str "pa_only", "inter_only", "both"
                :param inter_given: bool if group_info contains intersection
                :return: A numpy array of the corrected ranking"""

    if atr_correct == "pa_only":
        if inter_given:
            group_info = group_info[:-1,:]
        attribute_legend = list_of_attribute_dicts(group_info)
        corrected = ranking.copy()
        num_attributes = group_info.shape[0] - 1
        max_parity, min_parity, max_parity_group_id, min_parity_group_id, atrribute_groupids_sortedby_parity = get_max_min_par(
            corrected, attribute_legend, group_info)
        #print("max - min parity per attribute at start", np.asarray(max_parity) - np.asarray(min_parity))

        swap_num = 0

        while not np.all(np.asarray(max_parity[:-1]) - np.asarray(min_parity[:-1]) <= pthres):

            # bail out if swaps is greater than num_items
            if swap_num > ((len(ranking) * (len(ranking) - 1)) / 2):
                print("Try increasing the allowable difference in parity pa only")
                return corrected
                break
            attribute_to_correct = np.argmax(np.asarray(max_parity) - np.asarray(min_parity))
            top = find_candidate_lowest(corrected, max_parity_group_id[attribute_to_correct],
                                        attribute_legend[attribute_to_correct])
            index_top = np.argwhere(corrected == top)[0][0]
            bottom, index_top, top, index_bottom = find_bottom_candidate(corrected, index_top, attribute_to_correct,
                                                                         attribute_legend,
                                                                         min_parity_group_id[attribute_to_correct])
    ...
    if atr_correct == "both":
        ...
        while not np.all(np.asarray(max_parity[:-1]) - np.asarray(min_parity[:-1]) <= pthres) or not np.all(
                np.asarray(max_parity[-1]) - np.asarray(min_parity[-1]) <= ithres):

            # bail out if swaps is greater than num_items
            if swap_num > ((len(ranking) * (len(ranking) - 1)) / 2):
                print("Try increasing the allowable difference in parity both")
                return corrected
                break
            attribute_to_correct = np.argmax(np.asarray(max_parity) - np.asarray(min_parity))
            top = find_candidate_lowest(corrected, max_parity_group_id[attribute_to_correct],
    ...
    if atr_correct == "inter_only":
        ...
        while not np.all(np.asarray(max_parity[-1]) - np.asarray(min_parity[-1]) <= ithres):

            # bail out if swaps is greater than num_items
            if swap_num > ((len(ranking) * (len(ranking) - 1)) / 2):
                print("Try increasing the allowable difference in parity inter only")
                return corrected
                break
            attribute_to_correct = np.argmax(np.asarray(max_parity) - np.asarray(min_parity))
            top = find_candidate_lowest(corrected, max_parity_group_id[attribute_to_correct],
